 ...
 */
const _defaultPorts: { [key: string]: number | null } = {
  "ftp": 21,
  "file": null,
  "http": 80,
  "https": 443,
 ...
 */
export function setValidationErrorCallback(validationErrorCallback?: ((message: string) => void)) {
 ...
 * Creates a new URL.
 */
export function newURL(): URLRecord {
  return {
    scheme: '',
    username: '',
    password: '',
...
export function shorten(url: URLRecord): void {
  ...
  if (url.scheme === "file" && path.length === 1 &&
    isNormalizedWindowsDriveLetter(path[0])) return
...
export function hostParser(input: string, isNotSpecial = false): string | number | number[] | null {
   ...
   */
  const domain = utf8Decode(stringPercentDecode(input))
...
export function stringPercentDecode(input: string): Uint8Array {
   ...
   */
  return percentDecode(utf8Encode(input))
}
...
export function urlEncodedStringParser(input: string): [string, string][] {
   ...
   */
  return urlEncodedParser(utf8Encode(input))
}
...
export function urlEncodedSerializer(tuples: [string, string][], encodingOverride?: string): string {
  ...
  for (const tuple of tuples) {
     ...
     */
    const name = urlEncodedByteSerializer(utf8Encode(tuple[0]))
    /**
     ...
     */
    value = urlEncodedByteSerializer(utf8Encode(value))
    /**
